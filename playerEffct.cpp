...
void playerEffect::Draw(const Vector2& scroll) {
	for (int i = 1; i < EFFECT_MAX; i++) {
		...
		//Novice::DrawBox(static_cast<int>((CPos[i].x - size[i] / 2) - scroll.x), static_cast<int>((CPos[i].y - size[i] / 2) - scroll.y),static_cast<int>(size[i]), static_cast<int>(size[i]), 0.0f, color, kFillModeSolid);
